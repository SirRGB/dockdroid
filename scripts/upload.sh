...
_ota_info() {
  ...
  if [[ -z "$GITHUB_TOKEN" ]] && [[ -z "$GH_RELEASES_REPO" ]]; then
    custom_ota_url="https://github.com/${RELEASES_REPO}/releases/download/${TAG}/${PACKAGE_NAME}"
  else
...
if [[ -z "$GITHUB_TOKEN" ]] && [[ -z "$GH_RELEASES_REPO" ]]; then
  ...
  _print_upload_start
  _push_ota_info
elif [[ -f .ssh/id_* ]] && [[ -z "$SF_USER" ]] && [[ -z "$SF_RELEASES_REPO" ]]; then
  _print_upload_start
  _upload_sf
  _ota_info
  _print_upload_start
