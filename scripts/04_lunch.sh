...
_lunch() {
  ...
  if [[ -n "${LUNCH_PREFIX_FALLBACK}" ]]; then
    ...
    product=$(grep --extended-regexp "${TARGET_DEVICE}" "${ANDROID_BUILD_TOP}"/device/*/"${TARGET_DEVICE}"/AndroidProducts.mk | cut --delimiter='/' --fields=2 | cut --delimiter='.' --fields=1 | head --lines=1)
  fi
